...
function bech32Validator (network, address, opts) {
  ...
  if (witnessVersion !== 0 && witnessVersion !== 5 && witnessVersion !== 18 && witnessVersion !== 31) {
    console.log('Unsupported witness version for bech32')
    return false
  }

  if (opts.mainNetPrefix === 'zs' && opts.testNetPrefix === 'ztestsapling') {
    ...
    const data = bech32.fromWords(decoded.words.splice(1))
